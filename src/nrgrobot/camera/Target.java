...
public class Target {
    ...
    public static int k_thresholdSettings = CameraSettings.TARGET_THRESHOLD_STARTING_VALUE;
    public static void incrementThreshold()
    {
        k_thresholdSettings++;
    }
    public static void decrementThreshold()
    {
        if (k_thresholdSettings > CameraSettings.TARGET_THRESHOLD_MIN_VALUE)
            k_thresholdSettings--;
    }

    public static int getThreshold()
    {
        return k_thresholdSettings;
    }

    public static void setThreshold(int value)
    {
        k_thresholdSettings = value;
        if (k_thresholdSettings < CameraSettings.TARGET_THRESHOLD_MIN_VALUE)
            k_thresholdSettings = CameraSettings.TARGET_THRESHOLD_MIN_VALUE;
    }
    ...
    //The line below was commented out so that we can use an optimized EdgeThreshold value...
    //TODO: At competition, once we've tuned to the right constant, we will reinstate the line below :D
    //static CurveOptions curveOptions = new CurveOptions(0, CameraSettings.TARGET_THRESHOLD_STARTING_VALUE, 1, 25, 15, 15, 10, 1, 0);
    ...
    public static Target[] findCircularTargets(ColorImage image) throws NIVisionException { //include gregs other parameters?
        ...
        CurveOptions curveOptions = new CurveOptions(0, k_thresholdSettings, 1, 25, 15, 15, 10, 1, 0);
